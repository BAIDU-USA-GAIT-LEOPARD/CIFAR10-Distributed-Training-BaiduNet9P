...
rand_gpu_id = randint(0, n_gpus - 1)
...
def fit(epochs:int, learn:BasicLearner, callbacks:Optional[CallbackList]=None, metrics:OptMetrics=None)->None:
    ...
    try:
        for epoch in pbar:
            ...
            t_diff = t2-t1
            _total_time_train += t_diff
            if torch.cuda.current_device() == rand_gpu_id:
                with open('Perf_BaiduNet9P.tsv', 'a') as tsvfile:
                    tsvfile.write("%d	%.8f	" % (epoch, _total_time_train/3600))
...
class Recorder(LearnerCallback):
    ...
    def on_epoch_begin(self, **kwargs:Any)->None:
        if self.add_time: self.start_epoch = time.process_time()
    ...
    def on_epoch_end(self, epoch:int, num_batch:int, smooth_loss:Tensor,
        ...
        _accuracy_cifar10 = last_metrics[1].item()*100 
        if torch.cuda.current_device() == rand_gpu_id:
            with open('Perf_BaiduNet9P.tsv', 'a') as tsvfile:
                  tsvfile.write("%.2f\n" % (_accuracy_cifar10))
    ...
    def format_stats(self, stats:TensorOrNumList)->None:
        ...
        if not self.silent: self.pbar.write(str_stats, table=False)
